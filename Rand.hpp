...
public:
  ...
  Rand(unsigned long long w_ = 88675123):
...
inline unsigned long long Rand::next(){
...
inline void Rand::setBinary(MatD& mat){
  for (int i = 0; i < mat.rows(); ++i){
    for (int j = 0; j < mat.cols(); ++j){
      mat.coeffRef(i, j) = this->zero2one() > 0.5 ? 1.0 : 0.0;
    }
  }
}
